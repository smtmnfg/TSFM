...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        print("this is the start-index: ",border1)
        print("this is the end-index: ",border2)
...
class Dataset_FF(Dataset):
    ...
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='FF.csv',
                 target='actual_state', scale=True, timeenc=0, freq='s',
...
class Dataset_Pulp_minute(Dataset):

    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='PulpDataset.csv',
                 target='y', scale=True, timeenc=0, freq='t',
                 percent=10, max_len=-1, train_all=False):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
...
class Dataset_FF_Pred(Dataset):
    def __init__(self, root_path, flag='pred', size=None,
                 features='S', data_path='FF.csv',
                 target='actual_state', scale=True, inverse=False, timeenc=0, freq='1s', cols=None,
                 percent=None, train_all=False):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
...
class Dataset_Pulp_Pred(Dataset):
    def __init__(self, root_path, flag='pred', size=None,
                 features='S', data_path='PulpDataset.csv',
                 target='OT', scale=True, inverse=False, timeenc=0, freq='2min', cols=None,
                 percent=None, train_all=False):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
    ...
    def __read_data__(self):
        ...
        if self.timeenc == 0:
            ...
            df_stamp['hour'] = df_stamp.date.apply(lambda row: row.hour, 1)
            df_stamp['minute'] = df_stamp.date.apply(lambda row: row.minute, 1)
            df_stamp['minute'] = df_stamp.minute.map(lambda x: x // 2)
            data_stamp = df_stamp.drop(['date'], 1).values
        elif self.timeenc == 1:
            data_stamp = time_features(pd.to_datetime(df_stamp['date'].values), freq=self.freq)
            data_stamp = data_stamp.transpose(1, 0)
