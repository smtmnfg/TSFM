...
#SBATCH --output PulpDataset.out
#SBATCH --error PulpDataset.err
...
python -u /work/jayakodc/mcnair_data/smart-manufacturing/smart-manufacturing/custom_compact_models/NeurIPS2023-One-Fits-All/Anomaly_Detection/run.py \
  ...
  --is_training 1 \
  --root_path /work/jayakodc/mcnair_data/smart-manufacturing/smart-manufacturing/custom_compact_models/NeurIPS2023-One-Fits-All/Anomaly_Detection/datasets/PulpDataset \
  --model_id PulpDataset \
  --model GPT4TS \
  --data PulpDataset \
  --features M \
  --seq_len 100 \
